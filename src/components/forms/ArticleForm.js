...
const ArticleForm = ({ edit }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const res = edit
      ? await blogService.updateArticle(token, articleData, location.state.slug)
      : await blogService.createArticle(token, articleData)
    if (res.ok) {
      setLoading(false)
        ...
        <fieldset className="form__content form__content-lg">
          ...
          <label className="form__label">
            ...
            <textarea
              ...
              className="form__input form__input-lg form__textarea"
              placeholder="Text"
            />
          ...
          <label className="form__label form__label-tags">
            ...
            {fields.map((field, index) => (
              <div key={field.id} className="form__tags">
                ...
                />
                <button type="button" className="btn-error btn-lg-border" onClick={() => remove(index)}>
                  Delete
                </button>
                {fields.indexOf(field) === fields.length - 1 && fields.length !== 5 && (
                  <button type="button" className="btn-info btn-lg-border" onClick={() => append({ name: '' })}>
                    Add tag
                  </button>
                )}
                {errors?.tags?.[index]?.value && (
                  <p className="form__error" style={{ alignSelf: 'center' }}>
          ...
          {fields.length === 0 && (
            <button type="button" className="btn-info btn-lg-border form__addtag" onClick={() => append({ name: '' })}>
              Add tag
            </button>
          )}

          <Button loading={loading} htmlType="submit" className="form__button form__button-left" type="primary">
            Send
          </Button>
        </fieldset>
      </form>
